...
export default function NotesPage() {
  ...
  return (
    ...
    <div className="flex flex-row flex-wrap ml-5 mt-5 gap-4 bg-[#f9f5f1]">
      <button className="transition-colors ease-in-out duration-300 h-60 w-60 hover:bg-[#d49263] transition-colors bg-[#b97f55] rounded-2xl" onClick={() => setNotes([...notes, {}])}>
        <div className="text-white">New Note</div>
      </button>
      {notes.map((note, i) =>
        <div className="h-60 w-60 flex justify-center items-center hover:bg-[#d49263] transition-colors bg-[#b97f55] text-white rounded-2xl" onClick={() => document.getElementById('my_modal_1').showModal()}>Topic</div>
